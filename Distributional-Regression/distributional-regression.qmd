...
::: {.content-visible unless-format="revealjs"}
`tensorflow_probability` does all the hard work of mathematically describing the mixture distribution.
:::
...
```
1. Import `tfp` class from `tensorflow_probability`
2. Store statistical distributions in the `tfp` class as `tfd`
4. Extract predicted values for all model components and stores them in separate matrices
5. Specify the mixture distribution using computed model components
