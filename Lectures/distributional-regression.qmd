...
deep_glm = Sequential([
    ...
    Dense(64, activation="leaky_relu"),
    Dense(1, activation="exponential")                                  #<2>
])
...
2. The `exponential` activation plays the role of the GLM's inverse log link.
...
A mixture density network (MDN) [@bishop1994mixture] $\text{MDN}(\boldsymbol{x})$ outputs each distribution component's mixing weights and parameters of $Y$ given the input features $\boldsymbol{x}$, i.e.,
$$
...
alphas = Dense(n_components, activation="softplus")(x)
betas = Dense(n_components, activation="softplus")(x)
...
2. Specifies the output heads. `softmax` is used for the $\pi$ values as they must sum to 1. `softplus` is used for the $\alpha$'s and $\beta$'s as they must be positive.
...
## One package, every model

The [`drn`](https://github.com/EricTianDong/drn) package fits everything from today's lecture --- and the DRN itself --- behind a single `.fit` API:

```{python}
#| eval: false
from drn import GLM, CANN, MDN, DRN

glm = GLM("gamma").fit(X_train, y_train)                      # the GLM
cann = CANN(glm).fit(X_train, y_train)                        # the CANN
mdn = MDN("gamma", num_components=2).fit(X_train, y_train)    # the MDN

drn = DRN(glm).fit(X_train, y_train)                          # refine the whole distribution
```
...
# MDN
mdn_pred = gamma_mdn.predict(X_test, verbose=0)
nll_mdn = gamma_mixture_nll(y_test.values, keras.ops.convert_to_tensor(mdn_pred))
nll_mdn = float(nll_mdn)
