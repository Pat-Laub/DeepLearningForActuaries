...
#| echo: false
#| output: false
# THE data generation for the whole lecture. It lives up here, not at the
# case study slide where it is shown, because it must run before ANY python
# chunk reads data/splice/*.parquet — the motivating "one claim's life"
# slides read it ~700 lines before the case study, and knitr runs chunks in
# document order.
#
# Do NOT move this down / rewrite the parquet later in the render. Once a
# python chunk has read a parquet file, pyarrow keeps it mapped; rewriting
# that same file in place mid-session makes later reads in the same session
# hand back aliased garbage (payments came back carrying the estimates
# table's column names). Generate once, before the first reader.
#
# Unconditional: generation is fully reproducible (same random_seed => same
# claims, payments and case estimates in any fresh R session; verified
# 2026-07-14 by hashing all three tables across separate Rscript runs) and
# takes ~2.5s, so there is no fixture to protect and nothing to guard.
...
# Per-file "pre-render": compile interactive-splits/*.jsx to compiled/*.js
# (quarto pre-render hooks are project-wide only, and only this lecture needs
# the step). Silent no-op when compiled/ is up to date; chunk execution
# happens before quarto copies `resources:`, so fresh output always ships.
import subprocess
import sys
...
# Model caching is done inline at the NN slide (guarded keras save/load)
# rather than via a helper.
...
# nominal). Claim 1577 is hand-picked: incurred opens at $71k, two major
# revisions blow it out to $758k, then minor revisions walk it back down to
# the $473k ultimate — starts too low, overshoots, settles, the full story in
# one claim. Plotted over two slides (incurred view, then case estimate view)
# from this shared prep. NB this chunk reads the parquet BEFORE the case
# study's generation chunk runs; the setup chunk at the top of the file
# bootstraps it on a clean checkout. Uses the full-name imports from setup.py
# (pandas, plt) so the case study's visible `import pandas as pd` etc. slide
# is not preempted.
claim_story_no = 1577
...
Before any preprocessing: a claim is some static covariates plus a timeline of events. Step through a portfolio of 20 synthetic claims --- the _same_ claim is shown on every interactive slide in this lecture.
...
Each event date is binned into its calendar quarter, and we also track the _development quarter_ (quarters since the accident); the exact dates $d_k^A, d_k^N, d_k^F$ are then discarded, keeping only the quarters $(A_k, N_k, F_k)$.
...
Case estimates within a quarter are instead summarised by their _latest_ value (the estimate is a level, not a flow --- summing revisions would be meaningless).
...
#| echo: false
#| fig-dpi: 350
# Zoomed-out companion to the "total outstanding, visually" schematic — the
# same visual language (o = notification, x = payment, X = finalisation) but
# now a sample of real SPLICE claims, sorted by notification, with NO
# valuation line or highlighting: just claims overlapping in time.
_port = _claims.copy()
...
#| echo: false
#| output: false
# Snapshot features for the claims still open at the valuation date, built
# by re-running the snapshot pipeline with final_qtr clamped to T_END + 1
# (so the grid runs notif_qtr..T_END) and keeping each claim's final row.
# Mirrors the visible "Building the snapshot grid" / "History summaries"
# chunks — keep in sync if those change. Payments/estimates after T_END
# never merge in (the join key `quarter` stops at T_END), so no leakage.
...
The individual models can only reserve for claims they can _see_, so their fair target is the lower line; the chain ladder triangle implicitly projects the IBNR claims too, so its target is the upper one.
