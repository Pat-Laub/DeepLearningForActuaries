...
if not Path("nn_model_belgian.keras").exists():
    hist = nn_model.fit(X_train_nn, y_train_nn,
        epochs=25,
        batch_size=64,
        verbose=0,
    )
...
## Train loss {data-visibility="uncounted"}
...
## PDP: Alternate Reality II {.smaller data-visibility="uncounted"}
...
keras_reg = SKLearnRegressor(build_nn)
pipeline = Pipeline([
    ("preprocess", ct),
    ("regressor",  keras_reg)
])
pipeline.fit(X_train_raw, y_train_raw, regressor__verbose=0);
```
