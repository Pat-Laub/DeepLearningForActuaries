...
We give the model a few choices for the first two (the learning rate is continuous).
:::
...
::: {.content-visible unless-format="revealjs"}
The model runs through 10 random combinations of the hyperparameters and returns the one where the loss on the validation set is minimised. !!!CHECK WITH PL
:::
...
::: {.content-visible unless-format="revealjs"}
This time, we let the number of neurons in each hidden layer be tuned separately. For this example we are not tuning the activation functions or the learning rate. !!!CHECK WITH PL
:::
...
::: {.content-visible unless-format="revealjs"}
Bayesian search is not purely random, and updates its beliefs about the optimal hyperparameter values based on past models. Bayesian search balances *exploration* of new hyperparameter values and *exploitation* of the values that are expected to be optimal. !!!CHECK WITH PL
:::
