...
The `anam` package fits these with a Poisson/Gamma likelihood, an exposure offset, optional _monotonicity_ constraints, and automatic selection of features and pairwise interactions.
...
Trains an ensemble of shallow NAMs and scores each covariate by the _variance_ of its shape function.
...
Scores every pairwise term that passes the _strong heredity_ rule (a pair is only considered if both of its main effects are in the top $n\_main$), measuring how much each candidate improves the validation loss over the mains-only baseline.
...
Continuous features give _curves_; a categorical feature gives one value per level.
...
Measure the _standard deviation of each shape function across the training data_.
Exponentiating gives a _typical multiplier_ on the expected claim frequency.
