function PlotlySpark({
  ...
  React.useEffect(() => {
    ...
    const x = labels && labels.length === n ? labels : Array.from({ length: n }, (_, i) => `Q${i + 1}`);
    const yObs = values.map((v, i) => i <= cutoffIndex ? v : null);
    const yFut = values.map((v, i) => i >= cutoffIndex ? v : null);
    const mkHover = (v, i) => {
      const label = x[i];
      if (v == null) return `${label}: \u2014`;
      const val = currency && formatCurrency ? formatCurrency(v) : typeof v === "number" ? v.toFixed(2) : String(v);
      return `${label}: ${val}`;
    };
    ...
    const layout = {
      ...
      shapes: [{
        type: "line",
        xref: "x",
        // Use data coordinates instead of paper
        yref: "paper",
        x0: cutoffX,
        x1: cutoffX,
        y0: 0,
  ...
  return /* @__PURE__ */ React.createElement("div", { ref: elRef, className: "w-full" });
}
