...
![Preprocessing refers to the transformations of the raw data before input to the network](preprocessing.png)

Some steps are spectific to neural networks (e.g. standardising continuous variables), others are necessary for every model (e.g. encoding categorical variables, handling missing data).
...
A earlier check is to check that there are no duplicate rows in the dataset.
...
That is because the neural network expects continous inputs to be normalised to help with the training procedure.
...
X_train = imp.fit_transform(X_train_raw)    #<4>
X_val = imp.transform(X_val_raw)            #<5>
X_test = imp.transform(X_test_raw)          #<76>
```
::: {.content-visible unless-format="revealjs"}
...
6. Replace missing values in the validation set by the means of the training set columns.
7. Replace missing values in the test set by the means of the training set columns.
:::
...
2. Download the dataset from sklean. The `fetch_openml` function allows the user to bring in the datasets available in the OpenML platform. Every dataset has a unique ID, hence, can be fetched by providing the ID. `data_id` of the French motor dataset is 41214.
...
```
::: {.content-visible unless-format="revealjs"}
1. Splits the dataset in to train and test sets. By setting the `random_state` to a specific number, we ensure the consistency in the train-test split. `freq.drop("ClaimNb", axis=1)` removes the "ClaimNb" column.
...
# Example 3: Continous and Categorical Variables and Missing Values
...
nom_vars =  ["gender", "ever_married", "Residence_type",                                #<2>
    "work_type", "smoking_status"]                  

ct = make_column_transformer(
  (OneHotEncoder(sparse_output=False, handle_unknown="ignore"), nom_vars),              #<3>
...
for name, X in zip(("train", "val", "test"), (X_train, X_val, X_test)):        #<6>
...
preprocesor = joblib.load(PROCESSED_DATA_DIR / "preprocessor.pkl")
```
...
```{python}
preprocesor.transform(new_data)
```
