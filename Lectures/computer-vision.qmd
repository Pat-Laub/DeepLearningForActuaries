...
::: {.content-visible unless-format="revealjs"}
Computer vision is a field of Artificial Intelligence (AI) that focuses on extracting meaningful information from visual data (images and videos). One of the primary goals of computer vision is to correctly identify and classify visual data. Convolutional Neural Networks (CNNs) are the most commonly used neural network architectures for computer vision related tasks.
:::
...
::: {.content-visible unless-format="revealjs"}
Since the position of a pixel (one small square) in a photo can be represented using 3 positional values, we call it a rank 3 tensor.
:::
...
![An example of an image kernel in action.](images/convolution.gif)
...
![Example 3x3 filter](images/Glassner/16-9.png)
...
![Example: a 3x3 filter with 3 channels, containing 27 weights.](images/Glassner/16-18.png)
...
![Matching the original image footprints against the output location.](images/Glassner/16-10.png)
...
![What happens when filters go off the edge of the input?](images/Glassner/16-15.png)
...
![Padded values can be added to the outside of the input.](images/Glassner/16-17.png)
...
:::
::: column
![Example network highlighting that the number of output channels equals the number of filters.](images/Glassner/16-21.png)
:::
:::
...
![Example network with 1x1 convolution.](images/Glassner/16-23.png)
...
::: {.content-visible unless-format="revealjs"}
Striding allows us to modify the movement of the filter across the image. Instead of moving one step at a time (either horizontally or vertically), we can increase the number of steps using the *striding* option.
:::

We don't have to go one pixel across/down at a time.

![Example: Use a stride of three horizontally and two vertically.](images/Glassner/16-29.png)
...
::: {layout-ncol=2}
![](images/Glassner/16-31.png)

![](images/Glassner/16-32.png)
:::
...
:::
::: {.column width="40%"}
![](images/xkcd-trained_a_neural_net_2x.png)
:::
:::
...
![Typical CNN architecture.](images/CNN-architecture.png)
...
![](images/distill-feature-visualisation.png)
...
::: {.content-visible unless-format="revealjs"}
What if, rather than deep learning, we wanted to fit a logistic regression to an image?
:::
...
::: {.content-visible unless-format="revealjs"}
By flattening the image, we convert the matrix of information (tensor 2) into a vector (tensor 1), converting the dataset to tabular.
A consequence is that we lose information about the dimensions of the image.
:::
...
if Path("models/logistic.keras").exists():
    ...
    hist = model.fit(X_train, y_train, validation_data=(X_val, y_val),
      epochs=epochs, callbacks=[es], verbose=0)                         #<3>
...
2. Rescales the numeric representations of data which ranges from [0,255] into [0, 1] range
...
5. Applies the `Flatten` layer to convert the 2D array (from pooling) into a single column vector, and passes through couple of `Dense` layers to train the neural network for the specific classification problem.
...
```{python}
#| echo: false
warn = """ValueError                                Traceback (most recent call last)
Cell In[51], line 1
...
```{python}
X_test[0].shape, X_test[0][np.newaxis, :].shape, X_test[[0]].shape
```
...
![Or 'received wisdom' from experts...](images/twitter-advice.png)

:::
:::

::: footer
Source: [X](https://x.com/tetraduzione/status/1525501171221274627?s=20&t=ulVFesw-f7hCuG9KyaoYbQ).
:::
...
def objective(trial):
    ...
    model.save("_trial_model.keras")              #<1>
    ...
    return val_loss
```
1. Each trial's trained model is saved to Optuna's artifact store so the best model can be retrieved later
...
```{python}
best_id = study.best_trial.user_attrs["artifact_id"]
Path("best_random_model.keras").unlink(missing_ok=True)
optuna.artifacts.download_artifact(
    artifact_store=artifact_store,
    file_path="best_random_model.keras",
    artifact_id=best_id)
best_model = keras.models.load_model(
    "best_random_model.keras")
```

::: {.content-visible unless-format="revealjs"}
The model runs through 10 random combinations of the hyperparameters and returns the one where the loss on the validation set is minimised. After the search, we retrieve the best trial's saved model from the artifact store.
:::

## Tune layers separately {.smaller}
...
```{python}
sampler = optuna.samplers.TPESampler(seed=123) #<1>
study = optuna.create_study(
    direction="minimize",
    sampler=sampler)
...
study.optimize(objective, n_trials=10)
```
1. Optuna uses a _Tree-structured Parzen Estimator_ (TPE) by default, which is a Bayesian method. Passing a `seed` makes the search reproducible.
...
::: columns
::: column
![Example object classification run.](images/teach-images.png)
:::
::: column
...
::: {.content-visible unless-format="revealjs"}
Data augmentation: start with an image, and make new images from it by extracting certain information. Examples include: 

* De-texturising
* De-colorising
* Flipping/rotating

Data augmentation doesn't *add* new information, but it can make the model robust and sensitive to small changes in the images. In a way the model has more images to learn from.
:::

![Examples of data augmentation.](images/data-augmentation.png)
...
::: columns
::: {.column width="59%"}
![](images/inception-module-fig-2b.png)
:::
::: {.column width="39%"}

<br>

![](images/inception-meme.jpeg)
:::
:::
...
![Schematic of the GoogLeNet architecture.](images/Zhang-googlenet-architecture.png)

::: footer
Source: Zhang et al. (2018), [Can deep learning identify tomato leaf disease?](https://doi.org/10.1155/2018/6710865), Advances in Multimedia.
:::
...
::: footer
Source: Adapted from Goodfellow et al. (2014), [Multi-digit Number Recognition from Street View Imagery using Deep Convolutional Neural Networks](https://doi.org/10.48550/arXiv.1312.6082)
:::
...
ResNet won the ILSVRC 2014 challenge (top-5 error 3.6%), developed by [Kaiming He et al.](https://arxiv.org/abs/1512.03385) (see Figure 14-17 of Géron; 2018).
