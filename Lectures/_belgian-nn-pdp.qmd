...
# Compare to the ANAM
y_pred_anam = model_anam.predict(test_raw)
print(f"ANAM mean Poisson deviance: {mean_poisson_deviance(y_test_raw, y_pred_anam):.4f}")
print(f"ANAM MAE                  : {mean_absolute_error(y_test_raw, y_pred_anam):.4f}")
...
As recommended earlier, shuffle the _raw_ columns and score the composition $f \circ t$, so each categorical is swapped wholesale and no "multi-hot" rows are created.
...
## Permutation importance example {data-visibility="uncounted" .unlisted}

```{python}
#| eval: false
#| label: perm-test-nn-belgian
...
#| echo: false
#| output: false
#| label: perm-test-nn-belgian-cache
scores = load_or_compute(
    "models/perm_test_nn_belgian_raw.pkl",
    permutation_test_raw, nn_model, ct, X_train_raw, y_train_nn,
)
```
