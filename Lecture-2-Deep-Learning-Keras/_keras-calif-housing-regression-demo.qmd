...
2. Assigns features and target from the dataset to two variables 'features' and 'target' and return two separate data frames. The command `return_X_y=True` ensures that there will be two separate data frames, one for the features and the other for the target
...
The course focuses more on the modeling part of the life cycle.
:::
...
::: {.content-visible unless-format="revealjs"}
First, we split the data into the train set and the test set using a random selection.  By defining the random state, using the `random_state=42` command, we can ensure that the split is reproducible. We set aside the test data and assume we have never seen it. Then, we fit many models on the train data and select the one with the lowest train error. Thereafter we assess the performance of that model using the unseen test data.
:::
...
1. Imports the `LinearRegression` class from the `sklearn.linear_model` library
...
```
::: {.content-visible unless-format="revealjs"}
`X_train.head(3)` returns the first three rows of the dataset <i>X_train</i>.   
:::
...
```
::: {.content-visible unless-format="revealjs"}
`lr.predict(X_train.head(3))` returns the predictions for the first three rows of the dataset <i>X_train</i>.
:::

::: {.content-visible unless-format="revealjs"}
We can manually check for the accuracy of the `lr.predict()` function as follows. In the following code, we first define $w_0$ as the intercept of the `lr` function (initialization for 'prediction'), and then keep on adding the $w_i*x_i$ terms
:::

```{python}
prediction = lr.intercept_                              #<1>
for w_i, x_i in zip(lr.coef_, X_train.iloc[0]):         #<2>
    prediction += w_i * x_i                             #<3>
prediction                                              
```

1. Specifies the value of the intercept from the fitted regression as `prediction`
2. Iterates over the first observation from the train data (<i>X_train</i>) and the corresponding weight coefficients from the fitted linear regression
...
1. Imports `Sequential` from `tensorflow.keras.models`
2. Imports `Dense` from `tensorflow.keras.layers`
3. Defines the model architecture using `Sequential()` function

::: {.content-visible unless-format="revealjs"}
This particular neural network architecture consists of one hidden layer (30 neurons) and an output layer (1 neuron). While there is an activation function specified (`relu`) for the hidden layer, there is no activation function specified for the output layer. In situations where there is no specification, the output layer assumes a `linear` activation.
:::
...
```
::: {.content-visible unless-format="revealjs"}
Trying to train the same neural architecture with more epochs (epochs=50) to see if we can improve the results.
:::
## Loss curve
...
```
::: {.content-visible unless-format="revealjs"}
We should be mindful when selecting the activation function. Both `tanh` and `sigmoid` functions restrict the output values to the range of [0,1]. This is not sensible for house price modeling. `relu` does not have that problem. Also, `relu` ensures the output is positive which is realistic for house prices.
:::
...
::: {.content-visible unless-format="revealjs"}
We can see how the orginal values for the input varied between 0 and 10, and how the scaled input values are now between -2 and 2.5. Neural networks prefer if the inputs range between -1 and 1. 
:::
...
::: {.content-visible unless-format="revealjs"}
Early stopping can be seen as a regularization technique to avoid overfitting. The plot shows that both training error and valiation error decrease at the begning of training process. However, after a while, validation error starts to increase while training error keeps on decreasing. This is an indication of overfitting. Overfitting leads to poor performance on the unseen data, which is seen here through the gradual increase of validation error. Early stopping can track the model's performance through the trianing process and stop the training at the right time.
:::
::: footer
...
from tensorflow.keras.callbacks import EarlyStopping                            #<1>

random.seed(123)                                                                #<2>
model = Sequential([                                                            #<3>
    Dense(30, activation="relu"),                                               #<3>
...
1. Imports EarlyStopping from tensorflow.keras.callbacks
2. Sets the random seed
3. Constructs the sequential model
4. Configures the trianing process with optimizer and loss function
...
mc = ModelCheckpoint(                                                       #<2>
    "best-model.h5", monitor="val_loss", save_best_only=True                #<2>
)                                                                           #<2>
