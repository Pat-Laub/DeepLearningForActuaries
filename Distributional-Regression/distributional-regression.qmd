...
gamma_mdn = tf_keras.Model(inputs, tf_keras.layers.Concatenate(axis=1)([pis, alphas, betas]))    #<5>
```
...
```{python}
acts = model.get_layer("hidden")(X_train).numpy()
acts[:3]
```
...
Train $D$ neural networks with different random weights initialisations independently (can do it in parallel).

The trained weights are $\boldsymbol{w}^{(1)}, ..., \boldsymbol{w}^{(D)}$.
