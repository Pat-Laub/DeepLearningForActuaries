...
Play _Fizz Buzz_: count up, but say "Fizz" on multiples of 3, "Buzz" on multiples of 5, and "Fizz Buzz" on multiples of both.

![A vector-to-sequence RNN playing Fizz Buzz: one `START` input, the running count is the hidden state passed to the right, the spoken word is the output at each step.](images/fizz-buzz-rnn.svg)
