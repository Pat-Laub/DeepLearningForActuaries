...
# Standardising the Numerical Covariates {visibility="uncounted"}
...
# Handling Categorical Variables {visibility="uncounted"}
...
```{python}
print(f"In other words, {' < '.join(enc.categories_[0])}, and ")
...
```{python}
print(f"In other words, {' < '.join(enc_wrong.categories_[0])}.")
```
...
```{python}
print(f"In other words, {' < '.join(enc_right.categories_[0])}.")
```
...
## Group sparse categories {.smaller}
...
The `OneHotEncoder` can group infrequent levels for us.
...
A popular convention (from [Cookiecutter Data Science](https://cookiecutter-data-science.drivendata.org/)) splits `data/` by *how far along* the data is:
...
## Keep the preprocessor around {.smaller}
