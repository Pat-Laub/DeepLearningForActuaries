...
::: {.content-visible unless-format="revealjs"}
The following code uses the `transformers` library from _Hugging Face_ to create a text generation pipeline using the GPT-2 (Generative Pre-trained Transformer 2).
:::
...
3. Creates a pipeline object named `generator`, whose task would be to generate text using the pre-trained model.

```{python}
#| warning: false
transformers.set_seed(123)                                                              #<1>
print(generator("It's the holidays so I'm going to enjoy")[0]["generated_text"])        #<2>
```

1. Sets the seed for reproducibility
2. Applies the `generator` object to generate a text based on the input _It's the holidays so I'm going to enjoy_. The result from generator would be a list of generated texts. To select the first output sequence hence, we pass the command `[0]["generated_text"]`
...
## Question answering 
```{python}
qa = pipeline("question-answering", model="distilbert-base-cased-distilled-squad")      #<1>
```

1. Creates a _question and answer_ style pipeline object by referring to the pre-trained model _DistilBERT_ model (fine-tuned on the SQuAD: Stanford Question Answering Dataset).
...
::: {.content-visible unless-format="revealjs"}
_RLHF_: Reinforcement Learning from Human Feedback
:::
...
![It uses a fair bit of human feedback](images/ChatGPT-Diagram.png)
