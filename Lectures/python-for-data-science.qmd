...
X, y = make_regression(n_samples=100, n_features=4, random_state=0)         #<2>
...
2. Generates a toy dataset so the API is the focus, not any one problem.
3. Splits the dataset into train and test sets. `random_state=42` makes the split reproducible.
