...
Preprocessing of data is an essential step in the pipeline of creating a successful neural network. Good, proper preprocessing ensures that the data is in a format the neural network can learn from. 
:::
...
# French Motor Claims & Poisson Regression {visibility="uncounted"}
...
2. Brings in `fetch_openml` function from sklearn.datasets. `fetch_openml` allows the user to bring in the datasetls available in the OpenML platform. Every dataset has a unique ID, hence, can be fetched by providing the ID. `data_id` of the French motor dataset is 41214. 
...
::: {.content-visible unless-format="revealjs"}
Meaning that setting `activation="relu"` (with in strings) is same as setting `activation=relu` after bringing in the `relu` function from `tensorflow.keras.activations`. 
:::
...
```
::: {.content-visible unless-format="revealjs"}
This is similar to setting the activation function. Here, setting `optimizer="adam"` and `loss="poisson"` (with in strings) is same as setting `optimizer=Adam()` and `loss=poisson` after bringing in the `Adam` from `tensorflow.keras.optimizers` and `poisson` from `tensorflow.keras.losses`. Another important thing to note here is that, the loss function is no longer `mse`. Since we assume a poisson distribution for the target variable, and the goal is to optimize the algorithm for count data, poisson loss is more appropriate.
:::
...
```
::: {.content-visible unless-format="revealjs"}
Using `help` function in the above case, help us get information about the poisson loss function in `tensorflow.keras.losses` library. It shows that how `poisson` loss is calculated, by taking two inputs, (i) actual values and (ii) predicted values.
:::
...
3. Resets the index of train set, and drops the previous index column. Since index column gets shuffled during train-test split, it is good practice to reset the index. 
:::
...
::: {.content-visible unless-format="revealjs"}
`data["column_name"].value_counts()` function provides counts of each category for a categorical variable. In this dataset, variables `Area` and `VehGas` are assumed to have natural orderings where as `VehBrand` and `Region` are not not considered to have such natural orderings. Therefore, the two sets of categorical variables will have to be treated differently.
:::
...
3. Selects the two columns with ordinal variables from <i>X_train</i> and fits the ordinal encoder
...
```
::: {.content-visible unless-format="revealjs"}
We can extract each layer saparately from a trained neural network and observe its output given a specific input. 
...
::: column
```{python}
X_train["Region"].head(4)                       #<2>
```
```{python}
X_sample = X_train_reg[:4].to_numpy()           #<3>
X_sample
```
```{python}
enc_tensor = model.layers[0](X_sample)          #<4>
enc_tensor.numpy().squeeze()                    #<5>
```
:::
:::

::: {.content-visible unless-format="revealjs"}
1. Returns the weights of the Embedding layer. The function `model.layers[0].get_weights()[0]` returns a 22 $\times$ 2 weights matrix with optimal representations for each category. Here 22 corresponds to the number of unique catogories, and 2 corresponds to the size of the lower dimensional space using which we represent each category. 
...
::: {.content-visible unless-format="revealjs"}
While it not always the case, entity embeddings at times be meaningful instead of just being representations. The above figure shows how plotting the learned embeddings help reveal regions which might be similar (e.g. coastal areas, hilly areas etc.).
:::
...
::: {.content-visible unless-format="revealjs"}
Often times, we deal with both categorical and numerical variables together. The following diagram shows a recommended way of inputting numerical and categorical data in to the the neural network. Numerical variables are inherently numeric hence, do not require entity embedding. On the other hand, categorical variables must undergo entity embedding to convert to number format.
:::
...
::: {.content-visible unless-format="revealjs"}
The above code shows how to construct the same neural network using sequential models and Keras functional API. There are some differences in the construction. In the functional API approach, we must specify the shape of the input layer, and explicitly define the inputs and outputs of a layer. `model = Model(inputs, out)` function specifies the input and output of the model. This manner of specificying the inputs and outputs of the model allow the user to combine several inputs (inputs which are preprocessed in differnet ways) to finally build the model. One example would be combining entity embedded categorical variables, and scaled numerical variables.
:::
...
5. Fits the column transformer to the train set and tranforms it
6. Transforms the test set using the column transofrmer fitted using the train set
:::
...
```{python}
from tensorflow.keras.layers import Reshape                                     #<1>
...
::: {.content-visible unless-format="revealjs"}
1. Imports `Reshape` class from `tensorflow.keras.layers` library
2. Contructs the embedding layer by specifying the input dimension (the number of unique categories) and output dimension (the number of dimnesions we want the input to be summarized in to)
3. Reshapes the output to match the format required at the model building step
4. Contructs the embedding layer by specifying the input dimension (the number of unique categories) and output dimension
