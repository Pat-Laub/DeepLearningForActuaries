...
![Diagram of a word-level language model.](images/word-level-language-model.png)
...
1. Takes in the input text and generates the probability distribution of the next word. This distribution tells us how likely a certain word is to be the next word. 
2. Implements an appropriate sampling strategy to select the next word. 
3. Once the next word is predicted, it is appended to the input text and then passed in to the model again to predict the next word. 
...
![Diagram of a character-level language model (Char-RNN)](images/tensorflow-text_generation_sampling.png)
...
::: footer
Source: Table 1 of @hannun2014deep.
:::
...
plt.subplot(2, 2, 1)
temperature = 0.1
plt.bar(next_words, softmax(x, temperature), color=COLOURS)
...
![This is (probably) just the 'temperature' knob under the hood.](images/copilot-conversation-style.jpg)
...
![An example sequence-to-sequence chatbot model.](images/chatbot.png)
...
![Illustration of a beam search.](images/beam-search.png)
