...
![Example from @rudin2019.](scoring-rules-recidivism.png)
...
<!--
## Postcode is redundant {.smaller}

::: columns
::: {.column width="50%"}
```{python}
latlong_postcodes = claims[["lat", "long", "postcode"]].drop_duplicates()
latlong_postcodes
```
:::
::: {.column width="50%"}
...
```{python}
latlong_num_postcodes = latlong_postcodes.groupby(['lat', 'long']).aggregate({'postcode': 'count'})
latlong_num_postcodes
```

```{python}
latlong_num_postcodes.min(), latlong_num_postcodes.max()
```
...
## Plotting the GLM predictions II {data-visibility="uncounted"}
...
This is a [_ceteris paribus_ plot](https://christophm.github.io/interpretable-ml-book/ceteris-paribus.html), showing how the model's prediction changes as we vary the value of `ageph`, while keeping all other features constant at their values for the first training observation.
...
![Multiple conflicting explanations [@rudin2019]](saliency-criticism.jpg)
