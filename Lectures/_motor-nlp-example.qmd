...
```{python}
if not Path("data/NHTSA_NMVCCS_extract.parquet.gzip").exists():                               #<1>
    ...
    !wget -P data https://github.com/JSchelldorfer/ActuarialDataScience/raw/master/12%20-%20NLP%20Using%20Transformers/NHTSA_NMVCCS_extract.parquet.gzip
                                                                                                #<2>
df = pd.read_parquet("data/NHTSA_NMVCCS_extract.parquet.gzip")                                       #<3>
print(f"shape of DataFrame: {df.shape}")                                                        #<4>
```

1. Checks whether the zip folder already exists
2. If it doesn't, gets the folder from the given location
3. Reads the zipped `parquet` file and stores it as a data frame. `parquet` is an efficient data storage format, similar to `.csv`
4. Prints the shape of the data frame
...
df["NUMTOTV"].value_counts()\
    .sort_index()                   #<1>
```
1. The code selects the column with total number of vehicles `NUMTOTV`, obtains the value counts for each categories, and returns the sorted vector.
...
df["NUM_VEHICLES"].value_counts()\
  .sort_index()
```
1. Writes a function to reduce the number of categories to 3 by combining all categories with 3 or more vehicles into one category
:::
:::
...
```{python}
target_labels = df["NUM_VEHICLES"]                              #<1>
target = LabelEncoder().fit_transform(target_labels)            #<2>
target
```
1. Defines the target variable
2. Fit and transform the target variable using LabelEncoder
...
# Text Vectorisation with Bag of Words {visibility="uncounted"}
...
::::
:::: {.column width="40%"}
![Assigning Numbers](images/xkcd-assigning_numbers_2x.png){width="80%"}
::::
:::
...
start_of_summaries
```
1. Joins the words in the list with a space in between to return a string

## Count words in the first summaries

```{python}
vect = CountVectorizer()                                                #<1>
counts = vect.fit_transform(start_of_summaries)                         #<2>
vocab = vect.get_feature_names_out()                                    #<3>
print(len(vocab), vocab)                                                #<3>
```

1. `CountVectorizer` goes through a text document, identifies distinct words in it, and returns a sparse matrix. Here we apply `fit_transform` to the `start_of_summaries`
2. Stores the distinct words in the vector `vocab`
3. Returns the number of distinct words, and the words themselves
...
def vectorise_dataset(X, vect, txt_col="SUMMARY_EN", dataframe=False):              #<1>
    ...
    X_other = X[[f"WEATHER{i}" for i in range(1, 9)]]                               #<3>
...
3. Extracts just the numeric weather feature columns
...
```{python}
#| echo: false
# Collect each model's metrics for the comparison slide at the end.
model_results = []
_train_acc = model.evaluate(X_train_bow, y_train, verbose=0)[1]
_val_loss, _val_acc, _val_top2 = model.evaluate(X_val_bow, y_val, verbose=0)
model_results.append(("Bag of words (full vocab)", num_features,
    model.count_params(), _train_acc, _val_acc, _val_loss, _val_top2))
```
...
# Discard Infrequent Words and Stop Words {visibility="uncounted"}
...
::: {.content-visible unless-format="revealjs"}
This simplifies the problem, however, we might miss out on important words that might add value to the task. For example, _and_, _for_ and _of_ are among the selected words, but they are less meaningful.
:::
...
_train_acc = model.evaluate(X_train_bow, y_train, verbose=0)[1]
_val_loss, _val_acc, _val_top2 = model.evaluate(X_val_bow, y_val, verbose=0)
model_results.append(("Bag of words (top 1,000)", num_features,
    model.count_params(), _train_acc, _val_acc, _val_loss, _val_top2))
```
...
# Merge Together Similar Words {visibility="uncounted"}
...
```{python}
nlp = spacy.load("en_core_web_trf")                                         #<1>
doc = nlp("Apple is looking at buying U.K. startup for $1 billion")         #<2>
for token in doc:
    print(token.text, token.pos_, token.dep_, token.lemma_)                 #<3>      
```
1. Loads the model and stores it as `nlp`
2. Applies `nlp` model to the given string for processing. Processing involves tokenization, part-of-speech application, dependency application etc. 
3. Returns information about each token(word) in the line. `token.text` returns each word in the string, `token.pos_` returns the part-of-speech; the grammatical category of the word,  and `token.dep_` which provides information about the syntactic relationship of the word to the rest of the words in the string. 

## Dependency visualiser {.smaller}
...
<!-- 
## Dependency visualiser II

```python
doc = nlp(df["SUMMARY_EN"].iloc[2])
displacy.render(doc, style="dep")
``` -->


## Entity recognition {.smaller}
...
> — @vajjala2020practical
> :::

## Stemming and lemmatizing examples

![Examples of stemming and lemmatization](images/pnlp_0207.png)
...
::: {.content-visible unless-format="revealjs"}
Lemmatization refers to the act of reducing the words in to its base form. For example; reduced form of _looking_ would be _look_. The following code shows how we can lemmatize a text, by first processing it with `nlp`. 
:::
...
```
1. Starts defining the function which takes in a string of text as input
...
::: {.content-visible unless-format="revealjs"}
Lemmatized version of the column is now stored in `SUMMARY_EN_LEMMA`. We attach this new column to the existing _train_, _val_ and _test_ sets (matching on row index), so the same split and targets carry over --- only the text representation is new.
:::
...
X_train["SUMMARY_EN_LEMMA"] = df.loc[X_train.index, "SUMMARY_EN_LEMMA"]  #<1>
X_val["SUMMARY_EN_LEMMA"] = df.loc[X_val.index, "SUMMARY_EN_LEMMA"]
X_test["SUMMARY_EN_LEMMA"] = df.loc[X_test.index, "SUMMARY_EN_LEMMA"]

X_train.shape, X_val.shape, X_test.shape                                #<2>
```
1. Attaches the lemmatized summaries to the existing train/val/test sets, aligned by row index
2. Returns the dataset dimensions (now with an extra `SUMMARY_EN_LEMMA` column)
...
::: {.content-visible unless-format="revealjs"}
Training a NN using lemmatized datasets:
...
_train_acc = model.evaluate(X_train_bow, y_train, verbose=0)[1]
_val_loss, _val_acc, _val_top2 = model.evaluate(X_val_bow, y_val, verbose=0)
model_results.append(("Lemmatized BoW (top 1,000)", num_features,
    model.count_params(), _train_acc, _val_acc, _val_loss, _val_top2))
```
