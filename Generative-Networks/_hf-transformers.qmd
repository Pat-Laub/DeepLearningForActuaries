...
3. Creates a pipeline object named `generator`, whose task would be to generate text using the pre-trained model GPT-2. `revision="6c0e608"` specifies the specific revision of the model to refer
...
1. Creates a _question and answer_ style pipeline object by referring to the pre-trained model _DistilBERT_ model (fine-tuned on the SQuAD: Stanford Question Answering Dataset) with revision `626af31`
