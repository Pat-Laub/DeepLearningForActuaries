...
## Data science always starts with the data! {visibility="uncounted"}
...
## Train/validation/test split {visibility="uncounted"}
...
Keras is a common way of specifying, training, and using neural networks.
It gives a simple interface to _various backend_ libraries, including PyTorch. 
...
1. Defines a feed-forward model architecture
...
%time hist = model.fit(X_train, y_train, epochs=5, verbose=0)       #<4>
hist.history["loss"]                                                #<5>
```
...
By setting `verbose=0` we can avoid printing of intermediate results during training. Setting values of 1 or 2 can be useful when we want to observe how the neural network is training.  
:::
...
## Loss curve {data-visibility="uncounted" .unlisted}
...
::: {.content-visible unless-format="revealjs"}
@pace1997sparse studied the California house price dataset. This was one of the models they fit:
...
::: {.content-visible unless-format="revealjs"}
If you find that someone else has fit a model to the same dataset, you can compare your metrics to theirs in your report. For example, @pace1997sparse studied this dataset. You can compare the $R^2$ of their models to the ones we fit here.
:::
...
%time hist = model.fit(X_train_sc, y_train, epochs=1_000, \
    callbacks=[es], validation_data=(X_val_sc, y_val), verbose=0)           #<6>
