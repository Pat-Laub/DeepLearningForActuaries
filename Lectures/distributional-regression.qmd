...
::: {.content-visible unless-format="revealjs"}
This plot contains 300 simulated forecasts of the stock price over time. We see that the variance in the predictions increases with time. This makes sense as we are accumulating the random components over time.
:::
...
# Mean-Variance Network {visibility="uncounted"}

## Predicting the mean and the spread

Make a neural network that assumes $Y \mid \boldsymbol{x} \;\sim\; \mathcal{N}\bigl(\mu(\boldsymbol{x}),\, \sigma^2(\boldsymbol{x})\bigr)$.
...
# Regularisation and Dropout {visibility="uncounted"}
