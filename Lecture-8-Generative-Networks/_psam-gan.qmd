...
Encoding is the overall process of compressing an input with containing data in a high dimnesional space to a low dimension space. Compressing is the action of identifying necessary information in the data (versus redundant data)  and representing the input in a more concised form. The following slides show two different ways of representing the same data. The second representation is more concise (and smarter) than the first.
:::
...
model = keras.models.Sequential([
    layers.Rescaling(1./255, input_shape=(img_height, img_width, 1)),           #<2>
    layers.Flatten(),                                                           #<3>
...
7. Resclaes the input information back to the actual input scaling by multiplying with 255 (this completes the decoding stage, and now the input is in its original shape)
...
::: {.content-visible unless-format="revealjs"}
To further improve the process, we can try neural networks specialized for image processing. Here we use a Convolutional Neural Netowrk with convolutional and pooling layers. The following example shows how we first specify the encoder, and then the deconder. The two architectures are combined at the final stage.
:::
...
encoder = keras.models.Sequential([                                         #<2>
    layers.Rescaling(1./255, input_shape=(img_height, img_width, 1)),       #<3>
...
6. Performs maxpooling to reduce the dimnesion of the feature space
7. 
