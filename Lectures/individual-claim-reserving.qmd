...
Three main tables: one row per _claim_, per _payment_, and per _case estimate transaction_.
...
def make_preprocessor(with_estimate):
    ...
    return ColumnTransformer([
        ("dollars", make_pipeline(FunctionTransformer(np.log1p), MinMaxScaler()), dollars),
...
_Better%_ is the share of test claims we win vs the case estimates.
...
$$
  \text{Weighted Better\%} \;=\; \sum_{k} w_k \,\mathbf{1}\{\text{model wins claim } k\}, \qquad w_k = \frac{U_k}{\sum_j U_j} .
$$
...
#| echo: false
#| output: false
# Export the generated tables for reuse outside the render (labs, students).
# This must stay at the very end and stay write-only: nothing may read these
# files back in the same process, or pyarrow's mmap hands back corrupt strings.
from pathlib import Path
Path("data/splice").mkdir(parents=True, exist_ok=True)
claims.to_parquet("data/splice/claims.parquet")
payments.to_parquet("data/splice/payments.parquet")
estimates.to_parquet("data/splice/estimates.parquet")
```
