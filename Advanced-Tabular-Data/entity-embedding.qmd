...
X_train, X_test, y_train, y_test = train_test_split(                    #<2>
  freq.drop("ClaimNb", axis=1), freq["ClaimNb"], random_state=36861)    #<2>

# Reset each index to start at 0 again.
X_train_raw = X_train.reset_index(drop=True)                            #<3>
X_test_raw = X_test.reset_index(drop=True)                              #<3>
```
...
```{python}
X_train_brand = X_train["VehBrand"]; X_test_brand = X_test["VehBrand"]
X_train_region = X_train["Region"]; X_test_region = X_test["Region"]
...
```{python}
X_train_exp = X_train["Exposure"]; X_test_exp = X_test["Exposure"]    #<1>
X_train_rest = X_train.drop("Exposure", axis=1)                          #<2>
X_test_rest = X_test.drop("Exposure", axis=1)                            #<3>
```
