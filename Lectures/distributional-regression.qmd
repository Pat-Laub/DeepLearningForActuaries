...
3. [Take logs to get the log-likelihood.]{.step-loglik}

   $$
   \ell(\boldsymbol{\beta}) = \sum_{i=1}^n \log f(y_i; \theta_i)
   $$

4. [Negate it to get the objective to minimise.]{.step-nll}

   $$
   \text{NLL}(\boldsymbol{\beta}) = -\ell(\boldsymbol{\beta})
   $$

5. [Drop constants and rescale to get the model loss.]{.step-loss}
...
::: {.fragment}
Changing the distributional assumption changes $f$, so it changes the loss.
:::
...
so the claim amount $Y|\boldsymbol{X}=\boldsymbol{x}$ has density
$$
    \begin{align*}
