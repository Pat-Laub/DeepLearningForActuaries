...
if not Path("mandarin-split").exists():
    if not Path("mandarin").exists():
        !wget https://laub.au/ai/data/mandarin.zip
        !unzip mandarin.zip
...
model = keras.models.Sequential([
    ...
    layers.Flatten(),                                                           #<1>
    layers.Dense(num_hidden_layer, "relu"),                                     #<2>
    layers.Dense(img_height*img_width, "sigmoid"),                              #<3>
    layers.Reshape((img_height, img_width, 1)),                                 #<4>
])

model.compile("adam", "binary_crossentropy")
epochs = 1_000
es = keras.callbacks.EarlyStopping(patience=15, restore_best_weights=True)
model.fit(X_train, X_train, epochs=epochs, verbose=0,                           #<5>
    validation_data=(X_val, X_val), callbacks=es);                              
```

1. Flattens the monochrome image inputs into simple long vectors
2. Condenses the information from 6400 variables to 400 latent variables (the encoding stage ends here)
3. Converts the condensed representation from 400 to 6400 again. Note that the sigmoid activation is used to ensure output is between [0,1]
4. Converts the vector back into the original image shape (the decoding stage ends here)
5. Notice that the input is the same as the target, as we are trying to compress and reconstruct the original image.
...
## Some recovered image {visibility="uncounted" .unlisted}
