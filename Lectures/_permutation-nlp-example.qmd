...
#| echo: false
#| warning: false
#| label: fit-nlp-fail-model-cache
num_features = X_train_bow.shape[1]
num_cats = df_raw["NUM_VEHICLES"].nunique()
model = build_model(num_features, num_cats)
es = EarlyStopping(patience=1, restore_best_weights=True, monitor="val_accuracy")
if not Path("models/nlp-fail-model.keras").exists():
    model.fit(X_train_bow, y_train, epochs=1_000, callbacks=es,
        validation_data=(X_val_bow, y_val), verbose=0)
    model.save("models/nlp-fail-model.keras")
else:
    model = keras.models.load_model("models/nlp-fail-model.keras")
...
#| echo: false
#| output: false
#| label: perm-test-nlp-fail-cache
all_perm_scores = load_or_compute(
    "models/perm_test_nlp_fail.pkl",
    permutation_test, model, X_val_bow, y_val,
)
```
...
```{python}
clf = tree.DecisionTreeClassifier(random_state=0, max_leaf_nodes=3)     #<1>
clf.fit(X_train_bow[:, best_input_inds], y_train);                      #<2>
```
1. Specifies a decision tree with 3 leaf nodes. `max_leaf_nodes=3` ensures that the fitted tree will have at most 3 leaf nodes
2. Fits the decision tree on the selected dataset. Here we only select the `best_input_inds` columns from the train set
...
![Example decision tree for police reports on predicting the number of vehicles in a crash](images/nlp-decision-tree.png)
:::
...
```{python}
#| echo: false
#| label: fit-nlp-fixed-model-cache
if not Path("models/nlp-fixed-model.keras").exists():
    model.fit(X_train_bow, y_train, epochs=10, callbacks=es,
        validation_data=(X_val_bow, y_val), verbose=0)
    model.save("models/nlp-fixed-model.keras")
else:
    model = keras.models.load_model("models/nlp-fixed-model.keras")
```
...
#| echo: false
#| output: false
#| label: perm-test-nlp-fixed-cache
perm_scores = load_or_compute(
    "models/perm_test_nlp_fixed.pkl",
    permutation_test, model, X_val_bow, y_val,
)[:,1]
```
