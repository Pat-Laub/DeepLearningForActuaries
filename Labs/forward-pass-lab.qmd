...
where $\boldsymbol{X}$ is a $n \times d_{\boldsymbol{x}}$ matrix representing the weights, $\boldsymbol{w}$ is an $d_{\boldsymbol{x}} \times q$ matrix representing the weights ($q$ representing the number of neurons in the current layer), and $\boldsymbol{b}$ is an $n \times q$ matrix representing the biases. $n$ represents the number of observations and $d_{\boldsymbol{x}}$ represents the dimension of the input.
...
1. With the activations, weights, and activation functions given in the above figure and a constant bias of 1 for each node, calculate the values of \textcolor{red}{A}, \textcolor{red}{B}, \textcolor{red}{C}, and \textcolor{red}{D}.
2. If the \textcolor{red}{C} node represents ``YES" and the \textcolor{red}{D} node represents ``NO", what final value is predicted by the neural network?
