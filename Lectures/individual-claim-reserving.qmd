...
<!-- Numbers above are made up but internally consistent with the running
example (U_k = $48). -->
...
<!-- Chain ladder stays numeric-example-only: no general development-factor
notation (it collides with the already-heavy individual-level notation, and
students have seen the formal version). -->
...
All payments within a quarter collapse into one number, $Y_{k,t}$.
...
_port = (_port[_port["final_time"] <= 40]  # keep the axis inside the window
...
<!-- Target choice (outstanding vs ultimate) determines whether censored data
is worth anything — worth slowing down on. -->
...
<!-- This mimics reality: models are trained on the past, selected on the recent past, and evaluated on the most recent claims --- exactly how a deployed model would have been built over time. -->
...
<!-- Callback to the early-weeks leakage / grouped-splits material. The demo's
"duplication" toggle shows a censored train claim reappearing in val. -->
...
<!-- Note the quiet bias this creates: the test set only contains claims that _managed to finalise_ by $t_{\text{test}}$, so very long claims are under-represented. -->
...
<!-- BUILD: claim_no 1 spans ~21 quarters — check it fits the slide (else
.scrollable or a shorter claim). OCL/cumpaid are nominal (inflated) dollars. -->
...
observed = claims[(final <= T_END) & (final > notif)]
```
...
for name, pred in preds.items():
    ...
    print(f"{name:28s} test RMSE (log scale) = {rmse:.3f}")
```
